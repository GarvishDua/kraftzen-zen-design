  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <section className="pt-28 pb-16 relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 lg:px-8 relative z-10 text-center space-y-6 max-w-3xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-primary/10 border border-primary/20 mb-4">
              <Sparkles size={14} className="text-primary" />
              <span className="text-xs font-medium text-primary">Bro AI Ecosystem</span>
            </div>
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-display font-bold text-foreground">Our <span className="text-gradient-green">Products</span></h1>
            <p className="text-lg text-muted-foreground leading-relaxed mt-4">A suite of AI-powered tools designed to eliminate creative friction and supercharge your workflow.</p>
          </motion.div>
        </div>
      </section>
      ...
      <section className="py-20">
        <div className="container mx-auto px-4 lg:px-8 space-y-24">
          {products.map((p, i) => (
            <motion.div key={p.name} custom={i} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.2 }} variants={fadeUp} className={`grid md:grid-cols-2 gap-12 items-center ${i % 2 === 1 ? "md:direction-rtl" : ""}`}>
              <div className={`space-y-6 ${i % 2 === 1 ? "md:order-2" : ""}`}>
                <div className="w-14 h-14 rounded-2xl bg-primary/10 flex items-center justify-center">
                  <p.icon size={28} className="text-primary" />
                </div>
                <h2 className="text-3xl font-display font-bold text-foreground">{p.name}</h2>
                ...
                <Button className="rounded-full px-6 bg-primary hover:bg-primary/90 text-primary-foreground group">
              ...
              <div className={`relative ${i % 2 === 1 ? "md:order-1" : ""}`}>
                ...
                <img src={p.image} alt={`${p.name} - ${p.tagline}`} className="relative rounded-3xl shadow-2xl w-full h-72 object-cover" loading="lazy" />
              </div>
            </motion.div>
          ))}
        </div>
